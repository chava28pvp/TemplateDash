...
def handle_page(matclient, params):
    ...
    if mode in ("alarmado", "global", "severity") or is_integrity_pct_sort(sort):
        if not bool((params.get("options") or {}).get("allow_in_memory_sort", ALLOW_IN_MEMORY_SORT_FALLBACK)):
        ...
        return ok_response(
            ...
            total=len(scored),
            pagination={"page": page, "page_size": page_size, "offset": offset},
            meta={"mode": mode, "sorted_by": "server_computed"},
        )
...
def handle_integrity_baseline_week(matclient, params):
    fecha = params.get("fecha") or ((params.get("filters") or {}).get("fecha"))
    if not fecha:
...
def build_where(params, include_filter_keys=True, ignore_fecha=False, ignore_hora=False):
    ...
    fecha = params.get("fecha") or filters.get("fecha")
    hora = params.get("hora") or filters.get("hora")
    ...
    if include_filter_keys:
        add_in_filter(where, "Network", filters.get("networks") or filters.get("network"))
        add_in_filter(where, "Technology", filters.get("technologies") or filters.get("technology"))
        add_in_filter(where, "Vendor", filters.get("vendors") or filters.get("vendor"))
        add_in_filter(where, "Noc_Cluster", filters.get("clusters") or filters.get("cluster"))
